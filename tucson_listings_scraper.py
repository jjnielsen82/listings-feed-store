#!/usr/bin/env python3
"""
Tucson Listings Scraper - With GitHub Sync
Scrapes Tucson MLS active listings and syncs to GitHub.

Based on TucsonV2.py but simplified for listings data.
...
Usage:
    python3 tucson_listings_scraper.py
...
USERNAME = "tar.58407"
PASSWORD = "Babymj1214!"
LOGIN_URL = "https://tar.flexmls.com"
# UPDATE THIS to your active listings saved search ID
SEARCH_URL = "https://tar.flexmls.com/cgi-bin/mainmenu.cgi?cmd=url+search/saved/step2run.html&searchlist=20250818220351141583000000"
...
GITHUB_BRANCH = "main"
GITHUB_CSV_PATH = "data/tucson_listings.csv"
...
class GitHubSync:
    ...
    def sync_csv(self, local_rows: List[Dict], fieldnames: List[str]) -> int:
        ...
        message = f"Add {len(new_rows)} new Tucson listings ({ts})"
...
class TucsonListingsScraper:
    """Scrapes Tucson MLS active listings."""
    ...
    def login(self, page):
        """Login to Tucson FlexMLS using Clareity system."""
        print(f"[*] {timestamp()} - Logging in to Tucson MLS...")
        ...
        # Clareity uses contenteditable divs
        user_div = page.locator("#clareity")
        pw_div = page.locator("#security")

        if user_div.count() and pw_div.count():
            print(f"[*] {timestamp()} - Found Clareity login fields...")

            user_div.click()
            time.sleep(0.5)
            user_div.type(USERNAME, delay=100)
            time.sleep(0.8)

            pw_div.click()
            time.sleep(0.5)
            pw_div.type(PASSWORD, delay=100)
            time.sleep(0.8)
            ...
            try:
                page.evaluate("changeTheLoginButtonColor()")
            except:
                pass
            ...
            if login_btn.count():
                ...
                if is_disabled:
                    page.evaluate("""
                        ...
                        if (btn) {
                            btn.disabled = false;
                            btn.style.backgroundColor = '#092A56';
                            btn.style.color = '#FBFBFB';
                        }
                    """)
                    time.sleep(0.5)
        ...
        page.wait_for_load_state("networkidle", timeout=NAV_TIMEOUT)
        time.sleep(2)
        print(f"[✓] {timestamp()} - Login complete")
    ...
    def parse_html(self):
        ...
        for row in rows:
            ...
            # Parse agent (cell 8) and office (cell 9) for Tucson
            agent_info = {'name': '', 'phone': '', 'email': '', 'website': ''}
            office_info = {'name': '', 'phone': '', 'email': '', 'website': ''}

            if len(cells) > 8:
                agent_info = self.enhanced_contact_parser(str(cells[8]))
            if len(cells) > 9:
                office_info = self.enhanced_contact_parser(str(cells[9]))
    ...
    def run_extraction(self):
        ...
        print(f"\n{'='*60}")
        print(f"TUCSON LISTINGS SCRAPER - {timestamp()}")
        print(f"{'='*60}")

        ts = datetime.datetime.now(TZ).strftime("%Y%m%d_%H%M%S")
        out_html = self.output_dir / f"tucson_listings_{ts}.html"

        with sync_playwright() as p:
            browser = p.chromium.launch(headless=HEADLESS, slow_mo=SLOW_MO_MS, args=["--disable-gpu"])
            ...
            try:
                ...
                target.wait_for_load_state("domcontentloaded", timeout=NAV_TIMEOUT)
...
def main():
    print("\n" + "=" * 60)
    print("TUCSON LISTINGS SCRAPER - CONTINUOUS MODE")
    print("=" * 60)
    ...
    try:
        while True:
            run_count += 1
            print(f"\n{'*'*60}")
            print(f"* TUCSON LISTINGS RUN #{run_count} - {timestamp()}")
            print(f"{'*'*60}")

            try:
                scraper = TucsonListingsScraper()
